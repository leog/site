...
export const mdxComponents = {
  h1: ({ children, ...rest }: HeadingProps) => (
    <h1
      className='font-medium pt-12 mb-0 fade-in text-2xl'
      {...rest}
    >
      {children}
  ...
  p: ({ children, ...rest }: ParagraphProps) => (
    <p
      className='text-text leading-snug'
      {...rest}
    >
      {children}
    </p>
  ...
  ul: ({ children, ...rest }: ListProps) => (
    <ul
      className='text-text list-disc pl-5 space-y-1'
      {...rest}
    >
      {children}
    </ul>
  ...
  a: ({
    ...
    const className =
      "text-primary underline decoration-neon font-semibold";
    if (href?.startsWith("/")) {
      return (
        <Link
          href={href}
